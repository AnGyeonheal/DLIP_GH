...
Mat origin, src, src_gray, dst, dst_morph;
...
void Filter_Demo(int, void*){
    ...
    switch(filter_type){
        case 0: dst_morph.copyTo(dst_filtered); break;
        case 1: GaussianBlur(dst_morph, dst_filtered, cv::Size(ksize,ksize), 0, 0); break;
        case 2: medianBlur(dst_morph, dst_filtered, ksize); break;
        case 3:
            cv::Laplacian(dst_morph, temp, ddepth, kernel_size, scale, delta, cv::BORDER_DEFAULT);
            dst_morph.convertTo(dst_morph, CV_16S);
            dst_filtered = dst_morph - dst;
